import numpy as np
from geomat.library import LinearElastic, MCC, SMCC, C2MC, EMC, Elastoplastic
...
#         checks.check_inputs(self.get_model_name(), self.parameters.shape[0], self.state.shape[0], self.parameters_required, self.state_required)
...
#     # In the overriden method below the "state" variable passed is empty! There must be a conflict between the "state" in the base Model class and the shadowed "state" in the model implemenation (i.e. this class definition in Python)...
...
#     # In the method below passing and returning by reference needs to be solved. Also, the H_s and B_s vectors passed are empty because the "state" variable from which their size is derived internally is not the correct size either...

#     def compute_derivatives(self, sigma_prime, state, df_dsigma_prime, dg_dsigma_prime, H_s, B_s):
#         # State variables.
#         print("In compute_derivatives...")
#         print(sigma_prime)
#         print(state)
#         p_c = state[0]
