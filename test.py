import numpy as np
from models import LinearElastic, MCC, SMCC, C2MC, EMC
...
model = MCC(parameters=parameters, state=state, log_severity="info")
print("Model name: {}".format(model.name))
print("Model type: {}".format(model.type))
model.set_sigma_prime_tilde(stress)
p = np.zeros(increments)
...
model = SMCC(log_severity="info", parameters=parameters, state=state)
print("Model name: {}".format(model.name))
print("Model type: {}".format(model.type))
model.set_sigma_prime_tilde(stress)
p = np.zeros(increments)
