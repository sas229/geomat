...
void Elastoplastic::solve(void) { 
    ...
    if (f_1 <= FTOL) {
        ...
        if (!check_unload_reload(sigma_prime)) {
            ...
            Cauchy sigma_prime_0 = compute_isotropic_linear_elastic_trial_stress(sigma_prime, alpha_0, delta_epsilon_tilde);
            double f_0 = compute_f(sigma_prime_0);

            // Trial stress state for alpha_1.
            Cauchy sigma_prime_1 = compute_isotropic_linear_elastic_trial_stress(sigma_prime, alpha_1, delta_epsilon_tilde);
            double f_1 = compute_f(sigma_prime_1);
            ...
            alpha = compute_alpha(alpha_0, alpha_1, f_0, f_1);
        } else {
